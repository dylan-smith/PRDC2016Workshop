...
$restUrl = "https://$accountName.visualstudio.com/$tfsCollection/$teamProject/_apis/build/builds?definitions=$buildDefinitionId&resultFilter=succeeded&statusFilter=completed&`$top=1&api-version=2.0"
Write-Output "Executing REST API: $restUrl"
$build = Invoke-RestMethod $restUrl -Headers $headers

$buildId = $build.value.id

$restUrl = "https://$accountName.visualstudio.com/$tfsCollection/$teamProject/_apis/build/builds/$buildId/artifacts?api-version=2.0"
Write-Output "Executing REST API: $restUrl"
